    ...
    <main class="todos grid place-items-center">
        <form method="POST" class="w-full h-full flex items-center justify-center flex-col">
            ...
            w-4/6 h-full header flex flex-col justify-center items-center mt-20 shadow-gray-400 shadow-md p-10">
                ...
                w-5/6 mt-7 bg-white shadow-gray-600 shadow-sm h-10 text-black text-center rounded-full" placeholder="Nombre de la cultura">
                <input type="hidden" name="id" value="{{ isset($entidad) ? $entidad->idCultura : '' }}">
                <button type="submit" name="btnAgregarCultura" class="
                cel:w-32
                ...
                <table class="header w-full h-screen mt-20">
                    <thead class="flex justify-center items-center">
                        <tr class="cel:h-28
                            ...
                            <th class="text-3xl cel:text-lg">Lista de culturas descubiertas</th>
                        </tr>
                    </thead>
                    <tbody class="flex flex-col justify-start items-center">
                        @foreach ($aCultura as $cultura)
                        <tr class="w-11/12 flex justify-between items-center mt-10 mb-10 cel:mt-5 cel:mb-5">
                            <td class="cel:h-10 text-base font-thin w-9/12 h-16 bg-white grid place-items-center shadow-gray-500 shadow-md"> {{ $cultura->nombre}} </td>
                            <td class="cel:h-10 text-base font-thin w-2/12 h-16 bg-white grid place-items-center shadow-gray-500 shadow-md">
                                <a href="{{ route('culturas.eliminar', ['id' => $cultura->idCultura]) }}" class="cel:text-base text-3xl" name="btnEliminarCultura"><i class="fa-solid fa-trash"></i>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
